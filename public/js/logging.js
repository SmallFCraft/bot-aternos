    ...
    if (data.enabled) {
        contentElement.innerHTML = `
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 20px;">
                <div class="stat-card">
                    <div class="stat-number">${data.statistics?.totalRequests || 0}</div>
                    <div class="stat-label">Total Requests</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">${data.statistics?.successfulRequests || 0}</div>
                    <div class="stat-label">Successful</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">${data.statistics?.failedRequests || 0}</div>
                    <div class="stat-label">Failed</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">${data.statistics?.successRate || 0}%</div>
                    <div class="stat-label">Success Rate</div>
                </div>
            </div>
            
            <div style="margin-bottom: 15px;">
                <strong>Last Heartbeat:</strong> ${data.lastHeartbeat ? new Date(data.lastHeartbeat).toLocaleString() : 'Never'}
            </div>
...
async function testBetterStackHeartbeat() {
    try {
        ...
        if (data.success) {
            ...
            addLocalLog(`❌ Better Stack heartbeat test failed: ${data.error}`, 'error');
            alert(`❌ Heartbeat test failed: ${data.error}`);
        }
    } catch (error) {
        addLocalLog(`❌ Better Stack heartbeat test error: ${error.message}`, 'error');
...
function showBetterStackSetup() {
    const modalContent = `
        ...
        <form id="betterStackForm">
            ...
            <div class="form-group">
                ...
                <input type="number" id="checkInterval" class="form-input" min="30" max="3600" value="60">
            </div>

            <div class="checkbox-group">
                <input type="checkbox" id="enableBetterStack" class="checkbox" checked>
    ...
    document.getElementById('betterStackForm').addEventListener('submit', handleBetterStackSetup);
...
async function handleBetterStackSetup(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            addLocalLog(`❌ Failed to configure Better Stack: ${data.error}`, 'error');
