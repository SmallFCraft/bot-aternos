...
class Logger {
  constructor(logFilePath = "./logs/bot.log", maxLogSize = 5 * 1024 * 1024) {
    // 5MB max
    ...
    CommonUtils.ensureDirectory(this.logDir);
  ...
  rotateLogIfNeeded() {
    try {
      ...
      if (stats.size > this.maxLogSize) {
        ...
        const backupPath = this.logFilePath.replace(
          ".log",
          `_${timestamp}.log`
        );
        ...
        const initialLog = {
          timestamp: new Date().toISOString(),
          message: `Log rotated. Previous log saved as: ${path.basename(
            backupPath
          )}`,
          type: "info",
          source: "system",
