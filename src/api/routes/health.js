  ...
  router.get('/', (req, res) => {
    try {
      ...
      const httpStatus = healthStatus.status === 'healthy' ? 200 :
                        healthStatus.status === 'partial' ? 200 :
  ...
  router.get('/stats', (req, res) => {
    try {
      ...
      const botStats = {
        ...
        byServer: {},
        totalPackets: {
          sent: bots.reduce((sum, bot) => sum + (bot.status.packetsSent || 0), 0),
          received: bots.reduce((sum, bot) => sum + (bot.status.packetsReceived || 0), 0)
        },
        totalReconnects: bots.reduce((sum, bot) => sum + (bot.status.reconnectAttempts || 0), 0)
      };
