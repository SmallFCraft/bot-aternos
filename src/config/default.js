...
class Config {
  ...
  loadEnvironmentConfig() {
    ...
    this.defaultBot = {
      ...
      maxReconnectAttempts: parseInt(process.env.DEFAULT_MAX_RECONNECT_ATTEMPTS) || 50
    };
  ...
  validate() {
    ...
    if (this.betterStack.enabled) {
      if (!this.betterStack.heartbeatUrl) {
        errors.push('BETTER_STACK_HEARTBEAT is required when Better Stack is enabled');
  ...
  displaySummary() {
    ...
    console.log(`   • Default Server: ${this.defaultServer.host}:${this.defaultServer.port}`);
    console.log(`   • Default Username: ${this.defaultBot.username}`);
    console.log(`   • Minecraft Version: ${this.defaultServer.version}`);
    console.log(`   • Offline Mode: ${this.defaultBot.isOfflineMode ? 'Enabled' : 'Disabled'}`);
    console.log(`   • Better Stack: ${this.betterStack.enabled ? 'Enabled' : 'Disabled'}`);
    
    if (this.betterStack.enabled) {
      console.log(`   • Heartbeat Interval: ${this.betterStack.interval / 1000}s`);
    }
